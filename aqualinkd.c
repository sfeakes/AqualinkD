...
bool process_pda_packet(unsigned char* packet, int length)
{
  ...
  switch (packet[PKT_CMD]) {
    ...
    case CMD_MSG_LONG: {
      ...
      } else if (pda_m_type() == PM_SET_TEMP) {
        process_pda_packet_msg_long_set_temp(msg);
      } else if (pda_m_type() == PM_SPA_HEAT) {
        process_pda_packet_msg_long_spa_heat(msg);
      } else if (pda_m_type() == PM_POOL_HEAT) {
        process_pda_packet_msg_long_pool_heat(msg);
      } else if (pda_m_type() == PM_FREEZE_PROTECT) {
...
void main_loop() {
  ...
  if (_config_parameters.pda_mode == true) {
      _aqualink_data.frz_protect_set_point = 36;
      _aqualink_data.pool_htr_set_point = 90;
      _aqualink_data.spa_htr_set_point = 90;
      _aqualink_data.temp_units = FAHRENHEIT;
      set_pda_mode(true);
